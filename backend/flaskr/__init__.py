...
def create_app(test_config=None):
  ...
  def after_request(response):
    ...
    response.headers.add('Access-Control-Allow-Methods', 'GET, POST, DELETE') # No need to add the OPTIONS/PATCH/PUT Methods.
    return response
  ...
  @app.route('/categories') #I want this to work with any request as it's required for the Home page.
  ...
  @app.route('/questions') #I want this to work with any request as it's required for the Home page.
  def get_questions():
    page = request.args.get('page', 1, type=int)
    questions = Question.query.order_by(Question.id).paginate(page, 10 , False).items
    if len(questions) == 0:
        abort(404)
    questions_ready = format_data(questions) # helper function line 35 instead of repeating the next line over and over. 
  ...
  #I understand that the searchTerm is sent already within the body but I prefer it this way.
  ...
  def get_quiz_question():
    ...
    if category['id'] != 0:
        category_all = Category.query.filter_by(id=category['id']).one_or_none()
        if not category_all:
            abort(404)
