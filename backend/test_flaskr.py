...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_get_quiz_question(self):
        ...
        result = self.client().post('/quizzes', json={'quiz_category': {'type': 'Science', 'id': '1'}, 'previous_questions': []})
        data = json.loads(result.data)
        self.assertEqual(result.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_quiz_question_404(self):
        #post req with the required params.
        result = self.client().post('/quizzes', json={'quiz_category': {'type': 'none', 'id': '8888'}, 'previous_questions': []})
        data = json.loads(result.data)
        self.assertEqual(result.status_code, 404)
        self.assertEqual(data['success'], False)      
